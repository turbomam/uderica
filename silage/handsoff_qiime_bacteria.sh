...
# /home/mark/miniconda3/envs/qiime1/lib/python2.7/site-packages/matplotlib/__init__.py:784: UserWarning: Found matplotlib configuration in 
# ~/.matplotlib/matplotlibrc
# To conform with the XDG base directory standard, this configuration location has been deprecated on Linux, and the new location is now 
# /home/mark/.config/matplotlib/
# Please move your configuration there to ensure that matplotlib will continue to find it in the future.
# _get_xdg_config_dir())
# in either case, the setting for allowing remote/ssh instead of X windows is:
# backend : agg
...
if [ $INITIAL_MULTIQC == "TRUE" ]
  then
    MULTIQC_COMMAND="${MULTIQC_PATH} --config ${MULTIQC_CONFIG} ${PROJ_GENE_HOME}/${DATA_DIR}/fastqc -o ${PROJ_GENE_HOME}/${DATA_DIR}/multiqc"
...
if [ $DO_FLASH == "TRUE" ]
  ...
  while read p; do
    ...
    FLASH_COMMAND="${FLASH_PATH} ${CURRENT_R1} ${CURRENT_R2} -m ${MIN_FLASH_OVERLAP} -M ${MAX_FLASH_OVERLAP} -d ${FLASHED_DIR} -o ${p}"
    echo ${FLASH_COMMAND}
  ...
  mv ${FLASHED_DIR}/*.extendedFrags.fastq ${EXTENDED_DIR}
   ...
   for i in ${FLASHED_DIR}/*.hist ; do nawk '{print FILENAME"\t"$0}' $i ; done > ${FLASHED_DIR}/all.hist.txt
...
if [ $POST_FLASH_BOTHQC == "TRUE" ]
  then
    ...
    MULTIQC_COMMAND="${MULTIQC_PATH} --config ${MULTIQC_CONFIG} ${EXTENDED_DIR}/fastqc -o ${EXTENDED_DIR}/multiqc"
    echo ${MULTIQC_COMMAND}
...
if [ $DO_QIIME_SPLIT == "TRUE" ]
  then
    while read p; 
     ...
     sem --no-notice -j-1  split_libraries_fastq.py -q 29 -m ${QIIME_MAPPING_FILE} -i ${EXTENDED_DIR}/${associated}.extendedFrags.fastq  --sample_ids ${p} -o ${EXTENDED_DIR}/${associated}/ --barcode_type 'not-barcoded'  ";"
...
if [ $DO_QIIME_PICK == "TRUE" ]
  then
    ...
    # running 6 threads in parallel if possible, make that optional
...
Rscript /home/mark/gitstage/uderica/silage/handsoff_plots.R \
    ...
    --prefix=${PROJ_GENE_TITLE} >> ${LOG_FILE} 2>&1 
