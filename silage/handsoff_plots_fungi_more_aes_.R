...
if (experiment == "LalStress") {
  ...
  otufile <-
    paste0(
      '/home/mark/gitstage/uderica/silage/LalStress/Fungi/raw_data_fungi/flashed/extended/flash_trim_cat_pick/otu_table_mc2_w_tax.biom'
    )
  ...
  otufile <-
    paste0(
      '/home/mark/gitstage/uderica/silage/V-HMC/Fungi/raw_data_fungi/flashed/extended/flash_trim_cat_pick/otu_table_mc2_w_tax.biom'
    )
  ...
  otufile <-
    paste0(
      '/home/mark/gitstage/uderica/silage/Aer_Comp/Fungi/raw_data_fungi/flashed/extended/flash_trim_cat_pick/otu_table_mc2_w_tax.biom'
    )
}
...
hist(
  ...
  main = "Histogram of most specific rank... fix for fungi",
  xlab = "Taxonomic rank",
...
p <- plot_bar(phylo.manip, fill = "Family")
...
plist <- llply(as.list(ord_meths), function(i, phylo.manip, dist) {
  ...
  plot_ordination(phylo.manip, ordi, "samples", color = overrride)
...
dist <- phyloseq::distance(phylo.manip, "bray")
dist <- reshape2::melt(as.matrix(dist))
dist <- merge(x = dist,
              y = map.dat[, c("X.SampleID", "Treatment")],
              by.x = "Var1",
              by.y = "X.SampleID")
dist <- merge(
  x = dist,
...
aggdata <-
  aggregate(
    dist[, c("value")],
    by = list(dist$TreatmentNA, dist$TreatmentVar2),
    FUN = mean,
    na.rm = TRUE
  )
...
# p <- ggplot(pdataframe, aes(Axis_1, Axis_2, color = Treatment, fill = Treatment))
p <- ggplot(pdataframe, aes_(as.name(a1), as.name(a2), color = as.name(overrride), fill = as.name(overrride)))
# p
# p <- p + geom_point(size = 2, mapping = aes(shape = Treatment))
